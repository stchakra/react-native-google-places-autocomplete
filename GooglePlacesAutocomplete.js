...
export default class GooglePlacesAutocomplete extends Component {
  ...
  componentDidMount() {
    ...
    if(this.state.text == "" && this.props.predefinedPlaces2 != "Address"){
      debugger;
      console.warn("Address: "+ this.props.predefinedPlaces2);
      this.setState({text: this.props.predefinedPlaces2 })
    }
  }
        ...
        if (request.status === 200) {
          ...
          if (responseJSON.status === 'OK') {
            if (this._isMounted === true) {
                ...
                rowData.description = details.formatted_address;
              // try {
              ...
              //   }
              //   const zipSubtr1 = (getStringAfterSubstring(responseJSON.result.adr_address, '<span class="postal-code">'))
              //   var zipSubtr2 = (getStringBeforeSubstring(zipSubtr1, '</span>'));
              //   if(zipSubtr2.includes('-')){
              //     zipSubtr2 = (getStringBeforeSubstring(zipSubtr2, '-'));
              //   }
              //   // const zipSubtr3 = (getStringBeforeSubstring(zipSubtr2, '-'));
              //   rowData.description = rowData.description + ", " + zipSubtr2;
              // } catch (e) {
  ...
  _renderRowData = (rowData) => {
    ...
    return (
      <Text allowFontScaling={false} keyboardShouldPersistTaps={"handled"} style={[{ flex: 1 }, this.props.suppressDefaultStyles ? {} : defaultStyles.description, this.props.styles.description, rowData.isPredefinedPlace ? this.props.styles.predefinedPlacesDescription : {}]}
  ...
  _renderLoader = (rowData) => {
    if (rowData.isLoading === true) {
      return (
        <View keyboardShouldPersistTaps={'handled'} style={[this.props.suppressDefaultStyles ? {} : defaultStyles.loader, this.props.styles.loader]}>
          {this._getRowLoader()}
        </View>
      );
    }
  ...
  _renderRow = (rowData = {}, sectionID, rowID) => {
    return (
      <ScrollView
        ...
        >
          <View keyboardShouldPersistTaps={"handled"} style={[this.props.suppressDefaultStyles ? {} : defaultStyles.row, this.props.styles.row, rowData.isPredefinedPlace ? this.props.styles.specialItemRow : {}]}>
